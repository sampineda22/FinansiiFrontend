...
import { AXExpensesService } from './ax-expenses.service';

@Component({
    selector: 'app-ax-expenses',
    templateUrl: './ax-expenses.component.html',
    styleUrls: ['./ax-expenses.component.scss']
})
export class AXExpensesComponent implements AfterViewInit, OnDestroy {
    ...
    displayedColumns: string[] = ['number', 'expenseType', 'expenseCategory', 'seriesNum', 'invoiceId', 'gravadoAmount',
        'invoiceAmount', 'journalNum', 'invoiceDate', 'actions', 'eraseFilters'];
    title: string = '';
    subTitle: string = '';
    iconName: string = '';
    serieNum: string = '';
    ...
    journalNumUniqueValues: string[] = [];
    filteredJournalNumValues: string[] = [];
    ...
    constructor(private _translocoService: TranslocoService
        ...
        , private _axExpensesService: AXExpensesService
    ...
    ngOnInit() {
        this._translocoService.langChanges$.subscribe(() => {
            ...
            this.details = [];
            this.getExpensesOnAX();
        });
    }

    getExpensesOnAX() {
        this._axExpensesService.getAXExpenses$(this._sharedService.getCompanyCode()).subscribe(
            (data) => {
                if (data.data.length <= 0) {
                    Swal.fire("", "No se encontraron gastos aprobados", "info");
                    this.details = [];
                } else {
    ...
    putStatus(): void {
        this._axExpensesService.putStatus$(this._sharedService.getCompanyCode(), this.expenseDetailOpen.id, this.rejectionMotive).subscribe(
            (data) => {
                try {
                    ...
                    Swal.fire({
                        ...
                        text: `El detalle del gasto ha sido ${data.mensaje.toLowerCase()} exitosamente`,
                        icon: 'success',
                        customClass: {
    ...
    setUniqueValues(): void {
        ...
        this.journalNumUniqueValues = Array.from(new Set(this.details.map(item => item.journalNum).filter(value => value !== undefined)));
        this.filteredJournalNumValues = [...this.journalNumUniqueValues];
    ...
    filterJournalNumValues(searchText: string): void {
        this.filteredJournalNumValues = this.journalNumUniqueValues.filter(value =>
            value.toLowerCase().toString().includes(searchText.toLowerCase())
        );
    }
